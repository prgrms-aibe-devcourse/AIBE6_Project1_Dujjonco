...
class ReviewsService {
  ...
  async getReviewsByFacility(
    ...
    const reviews = data.map((item: any) => {
      ...
      return {
        ...
        is_liked: currentUserId ? item.review_likes?.some((like: any) => like.user_id === currentUserId) : false,
   ...
   */
  async createReply(reviewId: string, userId: string, content: string): Promise<{ success: boolean; error?: string }> {
    const { error } = await supabase
      .from('review_comments')
