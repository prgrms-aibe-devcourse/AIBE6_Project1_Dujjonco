...
async function uploadAllData() {
    const contentTypes = ['12', '32', '39'] // 12: 관광지, 32: 숙박, 39: 음식점
    const SERVICE_KEY = process.env.DATA_API_KEY
    const numOfRows = '20' // 상세정보까지 가져오니 20개가 적당.하루에 1000개만 긁어올 수 있어서 애매함
    const areaCodes = ['32', '33']
    /* (공공데이터 포털 지역코드 기준)
        지역명	코드	  지역명	코드
        서울	1	    인천	  2
        대전	3	    대구	  4
    ...
    */
    ...
    /*
        지역 1, 분류코드 12,32,39는 180개 이하 끝
        지역 31, 분류코드 32는 34개 , 12는 160개 , 39는 396개 끝

    */

    for (const areaCode of areaCodes) {
        for (const typeId of contentTypes) {
            let pageNo = 1 // 조회 시작 페이지
            let isFinished = false

            while (!isFinished) {
                console.log(`📡 [지역 ${areaCode} / 타입 ${typeId}] ${pageNo}페이지 시도 중...`)

                // @@페이지 넘으면 즉시 종료
                if (pageNo > 8) {
                    console.log(
                        `✅ [지역 ${areaCode} / 타입 ${typeId}] ${pageNo - 1}페이지까지 완료하여 다음 타입으로 넘어갑니다.`,
                    )
                    isFinished = true
                    break
                }
                ...
                try {
                    ...
                    if (!rawItems) {
                        console.log(`📭 [지역 ${areaCode} / 타입 ${typeId}] 더 이상 데이터가 없습니다.`)
                        isFinished = true
                        break
                    }
